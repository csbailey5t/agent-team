...
The goal is not just to find failures — it's to give a complete picture of where an application stands against accessibility standards and a clear path to improvement.
...
4. Check if Playwright is available (`which playwright` or check `package.json`) — enables runtime checks
